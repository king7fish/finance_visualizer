# ======================================================
# Finance Dashboard Ali v11.3 — Resilient + Intelligent + Universal Export
# All features restored and hardened for web:
...
# - Exports: CSV, XLSX, JSON, PNG, PDF, PPTX
#   with universal kaleido auto-detect and image fallback
# - Snapshot token (URL independent; no st.get_url dependency)
# - Robust cleaners to handle messy corporate files
...
# ---------------- PAGE CONFIG / THEME ----------------
st.set_page_config(page_title="Finance Dashboard Ali v11.3", layout="wide")
...
st.markdown(
    "<h1 style='text-align:center;color:#2563EB;'>Finance Dashboard Ali v11.3</h1>"
    "<p style='text-align:center;color:#6b7280;font-size:17px;'>Enterprise-grade, free-to-use data comparison with in-sheet intelligence.</p>",
    unsafe_allow_html=True,
)
...
def safe_to_datetime(series: pd.Series) -> pd.Series:
    """Vectorized datetime with robust fallback."""
    try:
...
def looks_numeric(s: pd.Series) -> bool:
    """Heuristic numeric detector tolerant of commas and symbols."""
...
def looks_date(s: pd.Series) -> bool:
    """Heuristic date detector using separators and month names."""
    vals = s.dropna().astype(str).head(100)
...
def smart_clean_dataframe(df_in: pd.DataFrame) -> pd.DataFrame:
    """Cleans DataFrame safely: trims headers, coerces numeric/date, strips text, drops empty rows."""
...
def detect_types(df: pd.DataFrame):
    ...
    for c in df.columns:
        s = df[c]
        if pd.api.types.is_datetime64_any_dtype(s) or looks_date(s.astype(str)):
            date_cols.append(c)
        elif pd.api.types.is_numeric_dtype(s) or looks_numeric(s.astype(str)):
            num_cols.append(c)
        else:
            txt_cols.append(c)
...
def split_sheet_into_tables(df: pd.DataFrame, min_blank_run: int = 1):
    """
    Split a sheet into multiple tables using blank rows; promote header-like first row.
    Returns list of (clean_table_df, 0, 0) for easy upstream usage.
    """
...
def load_files(files):
    """
    Returns dict: key -> DataFrame
    Keys include:
      - "file.xlsx - Sheet" (whole sheet)
      - "file.xlsx - Sheet - Table N" (in-sheet detected tables)
      - DOCX tables, PPTX slide tables, PDF text pages as tables
    """
    tables = {}
    for file in files:
        name = file.name
        ...
        try:
            if lower.endswith(("xlsx", "xls")):
                ...
                for s in xls.sheet_names:
                    ...
                    if subtables:
                        # keep a cleaned whole sheet too
                        df_try = xls.parse(s)
...
with tab_viz:
    ...
    if generate:
        with st.spinner("Preparing and aligning data..."):
            ...
            # X scaling only if numeric categories (dates are not scaled)
    ...
    if "ali_plotted" in st.session_state and not st.session_state["ali_plotted"].empty:
        ...
        if fig is not None:
            ...
            st.text_input("Copy this token and append to your app URL as ?snap=TOKEN",
                          value=token if token else "",
                          help="Example: https://yourapp.streamlit.app/?snap=<TOKEN>")
            st.markdown('</div>', unsafe_allow_html=True)
...
def fig_to_image_safe(fig, fmt="png"):
    """
    Try plotly + kaleido if available; otherwise return placeholder PNG/PDF.
    Works on Windows/macOS/Linux and Streamlit Cloud.
    """
    if fig is None:
        return placeholder_png("No Chart Available", (0, 0, 0))
    if st.session_state.get("disable_image_exports", False):
        return placeholder_png("Image export disabled", (255, 165, 0))
    try:
        import plotly.io as pio
        # kaleido==0.2.1 is pure-python and works on Streamlit Cloud
        return pio.to_image(fig, format=fmt, engine="kaleido")
    except ModuleNotFoundError:
        st.warning("Kaleido not found. Add 'kaleido==0.2.1' to requirements.txt for high-quality PNG/PDF.")
    except Exception as e:
        st.warning(f"Export fallback in use: {e}")
    # Fallback image placeholder (never crashes)
    return placeholder_png("Chart Export Placeholder", (255, 165, 0))
...
with tab_export:
    ...
    if "ali_plotted" not in st.session_state or st.session_state["ali_plotted"].empty:
        ...
        st.markdown("---")
        st.subheader("Export Chart (Universal Mode)")
        ...
        st.caption("Tip: For crisp PNG/PDF exports on the web, keep 'kaleido==0.2.1' in requirements.txt.")
    st.markdown('</div>', unsafe_allow_html=True)
