...
st.markdown(
    ...
    "<p style='text-align:center;color:#6b7280;font-size:17px;'>Data clarity. Visual power. Analyst precision.</p>",
...
def safe_to_datetime(series):
    """Convert any series to datetime without warnings."""
    try:
...
with tab_viz:
    if not tables:
        ...
        # === Label & Unit Controls ===
        st.subheader("🧩 Axis Labels & Units")
        c1, c2 = st.columns(2)
        with c1:
            custom_x_label = st.text_input("X-Axis Label", value=x_col)
            x_prefix = st.text_input("X-Axis Prefix", value="")
            x_suffix = st.text_input("X-Axis Suffix", value="")
        with c2:
            custom_y_label = st.text_input("Y-Axis Label", value=", ".join(y_cols) if y_cols else "")
            y_prefix = st.text_input("Y-Axis Prefix", value="")
            y_suffix = st.text_input("Y-Axis Suffix", value="")

        scale_choice = st.selectbox("Y-Axis Scale", ["None","Thousands (÷1,000)","Millions (÷1,000,000)","Billions (÷1,000,000,000)"])
...
with tab_settings:
    ...
    st.markdown("""
**Tips**
- Axis labels now support prefixes/suffixes for units (e.g. $, %, etc.).
- Ignore AI toggle gives full manual control.
- Layout optimised for clarity, speed, and minimal scroll. 🚀
""")
