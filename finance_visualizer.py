# finance_dashboard_elite_v9_1.py
...
# ------------------ PAGE CONFIG ------------------
st.set_page_config(page_title="Finance Dashboard Elite v9.1 — Killer Edition", layout="wide")
...
st.markdown(
    "<h1 style='text-align:center;color:#2563EB;margin-bottom:0;'>Finance Dashboard Elite v9.1</h1>"
    "<p style='text-align:center;color:#6b7280;font-size:17px;'>Corporate-grade analysis: aligned, normalized, and export-safe.</p>",
    unsafe_allow_html=True,
)
...
def safe_to_datetime(series):
    try:
        ...
        out = []
        for v in series:
            try: out.append(parser.parse(str(v)))
...
def common_frequency(selections):
    ...
    for df, x in selections:
        try: freqs.append(infer_pandas_freq(df[x]))
...
def align_on_calendar(dfs_info, target_freq):
    ...
    for info in dfs_info:
        ...
        if suffix: melt["Series"] = melt["Series"].astype(str) + " " + suffix
    ...
    if len(names) == 2:
        ...
        try:
            ...
            for lag in range(-3, 4):
                shifted = a_vals.shift(lag); c = shifted.corr(b_vals)
                if pd.notna(c) and c > best_corr: best_corr, best_lag = c, lag
            if best_corr != -2:
                if best_lag > 0: bullets.append(f"{a} leads {b} by {best_lag} period(s) (corr ~ {best_corr:.2f})")
                elif best_lag < 0: bullets.append(f"{a} lags {b} by {abs(best_lag)} period(s) (corr ~ {best_corr:.2f})")
                else: bullets.append(f"{a} and {b} move together (corr ~ {best_corr:.2f})")
        except Exception: pass
    else:
...
def compare_summary_table(df_long, value_col="Value"):
    ...
    out = pd.DataFrame({
        "Mean": safe.groupby("Series")[value_col].mean(),
        "Median": safe.groupby("Series")[value_col].median(),
...
with tab_viz:
    ...
    if "v9_plotted" in st.session_state and not st.session_state["v9_plotted"].empty:
        ...
        if fig is not None:
            ...
            snap_state = {
                "sources": st.session_state.get("v9_sources", []),
            ...
            try:
                base = st.get_url(); share_url = f"{base}?snap={token}"
            except Exception:
...
# ------------------ EXPORT TAB (NOW IN THE RIGHT PLACE) ------------------
with tab_export:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    if "v9_plotted" not in st.session_state or st.session_state["v9_plotted"].empty:
...
- Exports: CSV, Excel, JSON, PNG, PDF, PPTX (with graceful fallbacks).
""")
    st.markdown('</div>', unsafe_allow_html=True)
