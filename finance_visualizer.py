...
# ---------------- CONFIG ----------------
st.set_page_config(page_title="Finance Dashboard Elite v6", layout="wide")
PRIMARY = "#2563EB"
...
st.markdown(
    "<h1 style='text-align:center;color:#2563EB;margin-bottom:6px'>Finance Dashboard Elite v6</h1>"
...
with tab_viz:
    ...
    with st.expander("Units, Scaling, and Chart Type"):
    ...
    # Optional date filter if X is datetime
    ...
    # ---------------- Chart Controls specific to Bar/Pie ----------------
    bar_pie_card_opened = False
    if chart_type in ["Bar", "Pie"]:
        st.markdown('</div>', unsafe_allow_html=True)  # close previous card
        st.markdown('<div class="card">', unsafe_allow_html=True)
        bar_pie_card_opened = True
    ...
    try:
        if chart_type == "Pie":
            ...
            if compare_basis == "Totals per Series":
                ...
                # Choose one X category to slice pies on (for clarity)
                # If too many categories, show a select box
                ...
                if not cats:
                    ...
                    fig = px.pie(agg, names="Series", values="Value", color="Series",
                                 ...
                                 color_discrete_map=series_colors)
            else:  # Distribution by X category (percent)
                # Compute share of Series within each X category
                ...
                # Aggregate average share across categories (for a single pie view)
                ...
                fig = px.pie(agg, names="Series", values="SharePct", color="Series",
                             title="Average share across categories",
                             color_discrete_map=series_colors)
            ...
            if compare_basis == "Totals per Series":
                ...
                if layout_mode == "Overlay / grouped":
                    fig = px.bar(agg, x="Series", y="Value", color="Series",
                                 color_discrete_map=series_colors)
                else:  # Side-by-side is same as grouped since totals per series has no extra dimension
                    fig = px.bar(agg, x="Series", y="Value", color="Series",
                                 color_discrete_map=series_colors)
            elif compare_basis == "By X category (grouped)":
                # Bars grouped by category with Series hue
                ...
                if layout_mode == "Overlay / grouped":
                    ...
                    # Side-by-side small multiples (facet by Series)
                    fig = px.bar(grp, x=master_x_name, y="Value", facet_col="Series",
                                 color="Series", color_discrete_map=series_colors)
            else:
                # Distribution by X category: percent share by Series per category
                tmp = plotted.dropna(subset=["Value"]).copy()
                denom = tmp.groupby(master_x_name)["Value"].transform("sum")
                tmp["SharePct"] = np.where(denom > 0, (tmp["Value"] / denom) * 100.0, np.nan)
                ...
                if layout_mode == "Overlay / grouped":
                    fig = px.bar(tmp, x=master_x_name, y="SharePct", color="Series",
                                 barmode="group", color_discrete_map=series_colors)
                else:
                    fig = px.bar(tmp, x=master_x_name, y="SharePct", facet_col="Series",
                                 color="Series", color_discrete_map=series_colors)

        else:
            # Line, Area, Scatter use the long data directly
    ...
    if fig is not None:
        ...
        st.session_state["last_meta"] = {
            ...
            "sources": sources,
            "compare_basis": chart_type in ["Bar","Pie"] and compare_basis or None,
            "layout_mode": chart_type in ["Bar","Pie"] and layout_mode or None
        }
    else:
        st.info("Select at least one Y column to render a chart.")

    if bar_pie_card_opened:
        st.markdown('</div>', unsafe_allow_html=True)  # close Bar/Pie card
    st.markdown('</div>', unsafe_allow_html=True)  # close mapping card
...
- Bar: totals, grouped by category, or distribution (%). Overlay or side-by-side.
- Pie: totals per series, or a single category slice, or average distribution.
