...
st.markdown(
    ...
    "<p style='text-align:center;color:#6b7280'>Fast. Clean. Insightful.</p>",
...
def safe_to_datetime(series):
    """Efficient, warning-free datetime parser."""
    try:
...
def smart_clean_dataframe(df_in):
    """Clean messy data quickly and safely."""
...
def pptx_with_chart_failsafe(fig, title="Chart"):
    ...
    if img:
        ...
        slide.shapes.add_textbox(Inches(0.8), Inches(1.4), Inches(9), Inches(3)).text_frame.text = (
            "⚠️ Image export disabled or unsupported here."
        )
...
with tab_insight:
    if not tables:
        ...
        key = st.selectbox("Select dataset for insights", list(tables.keys()))
        df = tables[key].copy()
...
with tab_settings:
    ...
    st.markdown("""
**Notes**
- Option 3 ensures chart exports never crash.  
- Option 4 disables image exports entirely.  
- Heavy operations are cached for speed. 🚀
""")
